...
def known_versions(mocker):
    ...
    mocker.patch("west.core.files.get_known_versions", return_value=versions)

    return versions


def test_get_applied_versions(mocker, known_versions):
    mocker.patch("west.core.db.get_applied_versions", return_value=["1.0", "1.1"])
...
def test_get_best_schema_version_ok(mocker, known_versions):
    mocker.patch(
        "west.core.files.get_known_schemas",
...
def test_get_best_schema_version_ko(mocker, known_versions):
    ...
    with pytest.raises(exceptions.WestException):
...
def test_build_migration_plan_ok(mocker, known_versions):
    mocker.patch("west.core.db.get_applied_migrations", return_value=[])
    mocker.patch(
        "west.core.files.get_migrations_files_mapping",
        return_value={
            "file.ddl.sql": "tests/test_data/sql/17.1/manual/file.ddl.sql",
            "file.dml.sql": "tests/test_data/sql/17.1/manual/file.dml.sql",
        },
    )
    mocker.patch("west.core.files.is_manual_migration", return_value=True)
